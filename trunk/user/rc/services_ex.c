...
//	{ "TB.NETASSIST.UA",      "ipv6tb@netassist.ua"        },
	...
	{ "WWW.STRATO.COM",       "default@strato.com"         },
	{ "WWW.3322.ORG",         "dyndns@3322.org"            },
	{ "WWW.SPDYN.DE",         "default@spdyn.de"           },
	{ "WWW.FREEMYIP.COM",     "default@freemyip.com"       },
...
static const struct inadyn_checkip_t {
	const char *server;
	const char *path;
} inadyn_checkip_url[] = {
	{ NULL, NULL }, // default
...
write_inadyn_conf(const char *conf_file)
{
	...
	if (fp) {
		fprintf(fp, "secure-ssl = false\n"); // Currently we don't have ca-certificates.
		fprintf(fp, "period = %d\n", i_ddns_period);
		fprintf(fp, "forced-update = %d\n", i_ddns_forced);
		if (i_ddns_source != 0 && strlen(wan_ifname) > 0)
			fprintf(fp, "iface = %s\n", wan_ifname);
...
start_ddns(int clear_cache)
{
	...
	if (clear_cache)
		...
		inadyn_argv[9] = "-t";
		inadyn_argv[10] = (!is_ntpc_updated()) ? "15" : "3";
	}
	...
	i_ddns_verbose = nvram_safe_get_int("ddns_verbose", 6, 0, 16);
	snprintf(buf, sizeof(buf), "%d", i_ddns_verbose);
	inadyn_argv[8] = buf;

	return _eval(inadyn_argv, NULL, 0, NULL);
}
